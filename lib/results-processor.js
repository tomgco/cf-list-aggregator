...
var _ = require('lodash')
  , async = require('async')
  , isVisible = require('../visibility-check')

function createProcessor(serviceLocator) {
    ...
    if (!articles) {
      ...
      , function (item, eachCallback) {
          ...
          // if an overridden item, need to check if its live and if so, get all of the other data for this article
            ...
            if (isVisible(item)) {
              getSingleArticle(item.articleId, function (error, article) {
                if (article) {
  ...
  function getSingleArticle(articleId, callback) {
    articleId = serviceLocator.articleService.idType(articleId)
    serviceLocator.articleService.find({ _id: articleId }, function (error, results) {
