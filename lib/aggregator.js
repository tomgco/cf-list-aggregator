...
function createAggregator(crudService, options) {
  ...
  options.queryOverride = options.queryOverride
    || function (query) {return query}
  options.projectionOverride = options.projectionOverride
    || function (projection) {return projection}
  ...
  function aggregateList(list, cb) {
    ...
    // BEWARE: implementing this can destroy functionality of the aggregator.
    // 
    //                           YOU HAVE BEEN WARNED!
    // 
    // Allows a developer to override the query parameters and projection, this might be
    // a little bit scary as by doing this the developer can break the query
    // being made by the aggregator. However in certain situations this is needed.

    q.query = options.queryOverride(q.query)
    q.options = options.projectionOverride(q.options)
