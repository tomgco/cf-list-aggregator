...
function createAggregator(crudService, options) {
  ...
  options.queryOverride = options.queryOverride
    || function (query, projection) {return [query, projection]}
  ...
  function aggregateList(list, cb) {
    ...
    // 
    // Allows a developer to override the query parameters. This might be
    ...
    var queryOverride = options.queryOverride(q.query, q.options)
    q.options = queryOverride.pop()
    q.query = queryOverride.pop()
